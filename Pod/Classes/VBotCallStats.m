...
static NSString * const VBotCallStatsCodecImpairments = @"CodecImpairment";
static NSString * const VBotCallStatsBPL = @"BPL";
static NSString * const VBotCallStatsCodecFrameSize = @"CodecFrameSize";
NSString * const VBotCallStatsMOS = @"VBotCallStatsMOS";
NSString * const VBotCallStatsActiveCodec = @"VBotCallStatsActiveCodec";
NSString * const VBotCallStatsTotalMBsUsed = @"VBotCallStatsTotalMBsUsed";


@interface VBotCallStats()
...
@property (weak, nonatomic) VBotCall* call;
...
@implementation VBotCallStats

#pragma mark - Life Cycle

-(instancetype)initWithCall:(VBotCall *)call {
    if (self = [super init]) {
        self.call = call;
        ...
        self.codecSettings = @{
                               @"PCMA": @{
                                       VBotCallStatsCodecImpairments: @0.0,
                                       VBotCallStatsBPL: @4.3,
                                       VBotCallStatsCodecFrameSize: @0.125
                                       },
                               @"PCMU": @{
                                       VBotCallStatsCodecImpairments: @0.0,
                                       VBotCallStatsBPL: @4.3,
                               ...
                               @"G722": @{
                                       VBotCallStatsCodecImpairments: @0.0,
                                       VBotCallStatsBPL: @4.3,
                                       VBotCallStatsCodecFrameSize: @4.0
                                       },
                               @"iLBC": @{
                                       VBotCallStatsCodecImpairments: @32.0,
                                       VBotCallStatsBPL: @50.0,
                                       VBotCallStatsCodecFrameSize: @25.0
                                       },
                               @"Speex": @{
                                       VBotCallStatsCodecImpairments: @18.0,
                                       VBotCallStatsBPL: @21.0,
                                       VBotCallStatsCodecFrameSize: @30.0
                                       },
                               @"GSM": @{
                                       VBotCallStatsCodecImpairments: @28.0,
                                       VBotCallStatsBPL: @34.0,
                                       VBotCallStatsCodecFrameSize: @25.0
                                       },
                               @"opus": @{
                                       VBotCallStatsCodecImpairments: @11.0,
                                       VBotCallStatsBPL: @12.0,
                                       VBotCallStatsCodecFrameSize: @20.0
                                       },
                               };
...
- (NSDictionary *)generate{
    ...
    if (status == PJ_SUCCESS) {
        ...
        if (status == PJ_SUCCESS) {
            ...
            stats = @{
                      VBotCallStatsMOS: [[NSNumber alloc] initWithFloat:self.MOS],
                      VBotCallStatsActiveCodec: self.activeCodec,
                      VBotCallStatsTotalMBsUsed: [[NSNumber alloc] initWithFloat:self.totalMBsUsed]
                      };
        } else {
            VBotLogDebug(@"Unknown stream stat found");
            self.MOS = 0;
        }
    } else {
        char statusmsg[PJ_ERR_MSG_SIZE];
        pj_strerror(status, statusmsg, sizeof(statusmsg));
        VBotLogDebug(@"No stream found, status: %s", statusmsg);
...
- (void)codecUsed {
    if (self.streamInfo.type == PJMEDIA_TYPE_AUDIO) {
        ...
        VBotLogDebug(@"Stream is not an audio stream");
        self.activeCodec = @"Unknown";
    }
}
...
- (float)calculateEquipmentImpairment {
    NSNumber *codecImpairmentValue = [[self.codecSettings objectForKey:self.activeCodec] objectForKey:VBotCallStatsCodecImpairments];
    NSNumber *bplValue = [[self.codecSettings objectForKey:self.activeCodec] objectForKey:VBotCallStatsBPL];
 ...
 */
- (void)getCodecValues {
    NSNumber *frameSizeValue = [[self.codecSettings objectForKey:self.activeCodec] objectForKey:VBotCallStatsCodecFrameSize];
    ...
    VBotLogDebug(@"T: %f - Tr: %f - Ta: %f", self.T, self.Tr, self.Ta);
}
